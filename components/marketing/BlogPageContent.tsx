...
const FALLBACK_POSTS: BlogListItem[] = [
  {
    ...
    slug: "how-much-should-a-website-cost-for-a-small-business",
  ...
  {
    id: "fallback-growth",
    slug: "why-your-website-is-not-getting-customers",
  ...
  {
    id: "fallback-social",
    slug: "management-vs-marketing-what-is-the-difference",
...
export function BlogPageContent({ posts = FALLBACK_POSTS }: BlogPageContentProps) {
        ...
        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-3">
          {posts.map((post, index) => (
            <Link key={post.id} href={`/blog/${post.slug}`} className="block">
              ...
              >
                <p className="mb-2 text-center text-xs font-semibold uppercase tracking-[0.1em] text-[#6D5DF6]">{post.category?.name ?? "Blog"}</p>
                <h2 className="text-center text-xl font-bold text-[#0F172A]">{post.title}</h2>
                <p className="mt-3 text-center text-sm text-slate-600">{post.description ?? ""}</p>
                <span className="mt-5 inline-block text-sm font-semibold text-[#6D5DF6]">
                  Read article
                </span>
              </motion.article>
            </Link>
