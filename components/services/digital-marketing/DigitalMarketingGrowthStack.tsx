...
import { DIGITAL_MARKETING_GROWTH_STACK } from "@/lib/digital-marketing-service-data";
...
function GrowthBubblePill({ label, index }: { label: string; index: number }) {
  const reduceMotion = useReducedMotion();
...
export function DigitalMarketingGrowthStack() {
  const [activeIndex, setActiveIndex] = useState(0);
  const activeGroup = DIGITAL_MARKETING_GROWTH_STACK.groups[activeIndex];
                ...
                return (
                  <li key={group.category}>
                    <StackBubbleButton
                      ...
                      tabId={`digital-marketing-stack-tab-${index}`}
                      panelId={`digital-marketing-stack-panel-${index}`}
                    />
                  </li>
                );
              })}
          ...
          <Reveal delay={0.08} className="lg:col-span-8">
            <AnimatePresence mode="wait">
              <motion.article
                ...
                id={`digital-marketing-stack-panel-${activeIndex}`}
                role="tabpanel"
                aria-labelledby={`digital-marketing-stack-tab-${activeIndex}`}
              ...
              >
                ...
                <p className="mt-3 text-sm text-[#5C5C5C] md:text-base">
                  Tools and methods we use for {activeGroup.category.toLowerCase()} in growth projects.
                </p>

                <ul className="mt-8 flex flex-wrap justify-center gap-2.5 md:gap-3">
                  {activeGroup.items.map((item, index) => (
                    <GrowthBubblePill key={item} label={item} index={index} />
                  ))}
                </ul>
              </motion.article>
            </AnimatePresence>
