...
import { DEDICATED_TEAMS_FINAL_CTA } from "@/lib/dedicated-teams-service-data";

export function DedicatedTeamsFinalCta() {
  return (
      ...
      <div className="relative mx-auto max-w-[1440px] px-6 md:px-10">
        <Reveal className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#31C3C3]">
            {DEDICATED_TEAMS_FINAL_CTA.label}
          </p>
          <h2 className="mt-4 font-heading text-[clamp(2rem,4.2vw,3.25rem)] font-bold leading-[1.08] tracking-[-0.03em] text-[#FAFAF8]">
            {DEDICATED_TEAMS_FINAL_CTA.heading}
          </h2>
          <p className="mt-6 text-sm leading-relaxed text-white/60 md:text-base">
            {DEDICATED_TEAMS_FINAL_CTA.paragraph}
          </p>

          <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row sm:flex-wrap">
            <EditorialBoxCta href={DEDICATED_TEAMS_FINAL_CTA.primaryCta.href} variant="primary">
              {DEDICATED_TEAMS_FINAL_CTA.primaryCta.label}
            </EditorialBoxCta>
            <EditorialBoxCta href={DEDICATED_TEAMS_FINAL_CTA.secondaryCta.href} variant="on-dark">
              {DEDICATED_TEAMS_FINAL_CTA.secondaryCta.label}
            </EditorialBoxCta>
          </div>

          <p className="mt-10 text-xs font-medium uppercase tracking-[0.14em] text-white/40">
            {DEDICATED_TEAMS_FINAL_CTA.trustLine}
          </p>
        </Reveal>
      </div>
    </section>
