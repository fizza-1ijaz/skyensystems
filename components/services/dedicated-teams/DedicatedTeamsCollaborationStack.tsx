...
import { DEDICATED_TEAMS_COLLABORATION_STACK } from "@/lib/dedicated-teams-service-data";
...
function ToolBubblePill({ label, index }: { label: string; index: number }) {
  const reduceMotion = useReducedMotion();
...
export function DedicatedTeamsCollaborationStack() {
  const [activeIndex, setActiveIndex] = useState(0);
  const activeGroup = DEDICATED_TEAMS_COLLABORATION_STACK.groups[activeIndex];
      ...
      <div className="mx-auto max-w-[1440px] px-6 md:px-10">
        ...
        <div className="mx-auto mt-14 grid max-w-5xl gap-8 lg:grid-cols-12 lg:gap-12">
          <Reveal className="lg:col-span-4">
            ...
            <ul className="flex flex-col gap-3" role="tablist" aria-label="Dedicated team collaboration tools">
              {DEDICATED_TEAMS_COLLABORATION_STACK.groups.map((group, index) => {
                ...
                return (
                  <li key={group.category}>
                    <StackBubbleButton
                      ...
                      tabId={`dedicated-teams-stack-tab-${index}`}
                      panelId={`dedicated-teams-stack-panel-${index}`}
                    />
                  </li>
                );
              })}
          ...
          <Reveal delay={0.08} className="lg:col-span-8">
            <AnimatePresence mode="wait">
              <motion.article
                ...
                id={`dedicated-teams-stack-panel-${activeIndex}`}
                role="tabpanel"
                aria-labelledby={`dedicated-teams-stack-tab-${activeIndex}`}
              ...
              >
                ...
                <p className="mt-3 text-sm text-[#5C5C5C] md:text-base">
                  Tools we use for {activeGroup.category.toLowerCase()} in dedicated team delivery.
                </p>

                <ul className="mt-8 flex flex-wrap justify-center gap-2.5 md:gap-3">
                  {activeGroup.items.map((item, index) => (
                    <ToolBubblePill key={item} label={item} index={index} />
                  ))}
                </ul>
              </motion.article>
            </AnimatePresence>
