        ...
        <div className="flex flex-wrap gap-3">
          {categories.map((cat) => (
            <a 
              ...
              className="px-4 py-2 rounded-full glass border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all"
            >
              {cat.title}
            </a>
          ))}
        ...
        {categories.map((category, categoryIndex) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="p-4 rounded-2xl bg-slate-900 text-blue-400 shadow-xl">
                  <Icon size={32} />
                </div>
                <div>
              ...
              >
                {category.items.map((item, itemIndex) => (
                  ...
                  >
                    ...
                    <h3 className="text-2xl font-bold text-slate-900 mb-4 pr-12 relative z-10">{item.name}</h3>
                    <p className="text-slate-600 leading-relaxed mb-8 relative z-10">{item.description}</p>
                    
                    <div className="mt-auto space-y-4 relative z-10">
                      <div className="text-xs font-bold uppercase tracking-wider text-blue-600">Key Features</div>
                      <ul className="grid grid-cols-1 gap-3">
                        {item.features.slice(0, 4).map((feature) => (
                          <li key={feature} className="flex items-start gap-2 text-sm text-slate-500">
                            <div className="mt-1 p-0.5 rounded-full bg-blue-50 text-blue-600">
                              <Check size={12} />
                            </div>
                            {feature}
                          </li>
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between relative z-10">
                      ...
                      <button className="p-2 rounded-full border border-slate-100 text-slate-400 hover:bg-slate-50 hover:text-blue-600 transition-colors">
        ...
        <div className="relative overflow-hidden bg-slate-900 rounded-[3rem] p-12 md:p-24 text-center text-white shadow-2xl">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              <Link 
                href="/contact-us" 
                className="px-10 py-5 rounded-full bg-blue-600 text-white font-bold text-lg hover:bg-blue-500 hover:scale-105 transition-all shadow-xl shadow-blue-500/20"
              >
                Contact Our Team
              </Link>
              <Link 
