...
function RelatedWorkCard({
  ...
  card: (typeof DEDICATED_TEAMS_RELATED_WORK.cards)[number];
  index: number;
}) {
      ...
      <div className="mx-auto max-w-[1440px] px-6 md:px-10">
        <Reveal className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#31C3C3]">
            {DEDICATED_TEAMS_RELATED_WORK.label}
          </p>
          <h2 className="editorial-section-title mt-4 text-balance text-[#141414]">
            {DEDICATED_TEAMS_RELATED_WORK.heading}
          </h2>
          <p className="mt-4 text-sm leading-relaxed text-[#5C5C5C] md:text-base">
            {DEDICATED_TEAMS_RELATED_WORK.paragraph}
          </p>
        </Reveal>

        <div className="mt-14 grid gap-5 md:grid-cols-3 md:gap-6">
          {DEDICATED_TEAMS_RELATED_WORK.cards.map((card, index) => (
